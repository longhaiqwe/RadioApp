            ...
            GeometryReader { geometry in
                ScrollViewReader { proxy in
                    ...
                    .onAppear {
                        ...
                        self.currentTime = matcher.currentSongTime
                    }
                    .background(
                        TimelineView(.periodic(from: .now, by: 0.5)) { context in
                            Color.clear
                                .onChange(of: context.date) { _, _ in
                                    let newTime = matcher.currentSongTime
                                    self.currentTime = newTime
                                    print("LyricsView: TimelineView tick: currentTime=\(String(format: "%.2f", newTime))s, activeLine=\(lyricLines.last(where: { $0.time <= newTime })?.text ?? "none"), lyricLines count=\(lyricLines.count)")

                                    // Auto-scroll only if user is not interacting
                                    if !isUserScrolling {
                                        if let activeLine = lyricLines.last(where: { $0.time <= newTime }) {
