...
class StreamSampler: NSObject, URLSessionDataDelegate {
    ...
    private var directCaptureStartTime: Date?
    ...
    func sampleStream(from urlString: String, completion: @escaping (URL?, Bool, TimeInterval) -> Void) {
        ...
        self.directCaptureStartTime = Date()
    ...
    private func sampleHLSStream(from url: URL) {
        ...
        session.dataTask(with: request) { [weak self] data, response, error in
            ...
            if totalSegments > playerLagSegments {
                ...
                self.hlsSegmentOffset = totalDuration - lagDuration
                print("StreamSampler: HLS 偏移量计算: 总时长 \(String(format: "%.1f", totalDuration))s, 播放器落后 \(playerLagSegments) 个片段 (约 \(String(format: "%.1f", lagDuration))s), 净偏移量 \(String(format: "%.1f", self.hlsSegmentOffset))s")
            } else {
    ...
    private func saveAndComplete(data: Data, fileExtension: String) {
        ...
        do {
            ...
            // 如果是非 HLS 直接流，计算动态偏移量
            if !self.isHLSStream_ {
                let asset = AVURLAsset(url: url)
                let rawAudioDuration = CMTimeGetSeconds(asset.duration)
                if rawAudioDuration.isFinite && rawAudioDuration > 0 {
                    // 因为 ShazamMatcher 限制了最长只读取 12.0s，所以用于计算偏移的时长也必须限制在 12.0s 内
                    let audioDuration = min(12.0, rawAudioDuration)
                    let physicalDuration = Date().timeIntervalSince(self.directCaptureStartTime ?? Date())
                    self.hlsSegmentOffset = max(0, audioDuration - physicalDuration)
                    print("StreamSampler: 直接流(MP3/AAC) 偏移量计算: 原始时长 \(String(format: "%.1f", rawAudioDuration))s, 限额时长 \(String(format: "%.1f", audioDuration))s, 物理下载耗时 \(String(format: "%.1f", physicalDuration))s, 净偏移量 \(String(format: "%.1f", self.hlsSegmentOffset))s")
                } else {
                    self.hlsSegmentOffset = 0
                }
            }
